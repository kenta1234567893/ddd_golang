...
// The CustomerBankFunc type is an adapter to allow the use of ordinary
// function as CustomerBank mutator.
type CustomerBankFunc func(context.Context, *ent.CustomerBankMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CustomerBankFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CustomerBankMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CustomerBankMutation", m)
}
