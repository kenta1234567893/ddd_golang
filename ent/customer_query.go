...
type CustomerQuery struct {
	...
	withCustomerBanks *CustomerBankQuery
...
// QueryCustomerBanks chains the current query on the "customer_banks" edge.
func (cq *CustomerQuery) QueryCustomerBanks() *CustomerBankQuery {
	query := (&CustomerBankClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CustomerQuery) FirstIDX(ctx context.Context) uint64 {
...
func (cq *CustomerQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CustomerQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (cq *CustomerQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (cq *CustomerQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withCustomerBanks: cq.withCustomerBanks.Clone(),
...
// WithCustomerBanks tells the query-builder to eager-load the nodes that are connected to
// the "customer_banks" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CustomerQuery) WithCustomerBanks(opts ...func(*CustomerBankQuery)) *CustomerQuery {
	query := (&CustomerBankClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCustomerBanks = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withCustomerBanks; query != nil {
		if err := cq.loadCustomerBanks(ctx, query, nodes,
			func(n *Customer) { n.Edges.CustomerBanks = []*CustomerBank{} },
			func(n *Customer, e *CustomerBank) { n.Edges.CustomerBanks = append(n.Edges.CustomerBanks, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CustomerQuery) loadCustomerBanks(ctx context.Context, query *CustomerBankQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *CustomerBank)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Customer)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.CustomerBank(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.CustomerBanksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_customer_banks
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_customer_banks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_customer_banks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CustomerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(customer.Table, customer.Columns, sqlgraph.NewFieldSpec(customer.FieldID, field.TypeUint64))
	_spec.From = cq.sql
